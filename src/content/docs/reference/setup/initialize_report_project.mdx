...
If a `standard_footnotes.yaml` is not found within the `report_dir_name` subdirectory, a prompt stating:

```r
> Copied standard_footnotes.yaml into /cluster-data/user-homes/user/project/report_dir_name
```
...
- **Info**: Logs when the `project_dir` is validated, when `report_dir_name` and `outputs_dir_name` subdirectories are created, and when the `standard_footnotes.yaml` file is copied over.
...
- **Initialize Python Reference**: Refer to the reference on `initialize_python()` <a href={`${import.meta.env.BASE_URL}/reference/setup/initialize_python/`}>here</a>.
